    ...
    class AWS
      ...
      class Mock
        ...
        def attach_network_interface(nic_id, instance_id, device_index)
          ...
          if ! self.data[:instances].find{ |_i,i_conf|
            i_conf['instanceId'] == instance_id
          }
            raise Fog::Compute::AWS::NotFound.new("The instance ID '#{instance_id}' does not exist")
          elsif self.data[:network_interfaces].find { |_ni,ni_conf| ni_conf['attachment']['instanceId'] == instance_id && ni_conf['attachment']['deviceIndex'] == device_index }
